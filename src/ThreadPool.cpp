...
ThreadPool::ThreadPool(long minNumOfThread, long maxNumOfThread)
{
    if (minNumOfThread < 2)
...
void ThreadPool::CreateIdleThread(long size)
{
    idleThreadLock.Lock();
...
void ThreadPool::DeleteIdleThread(long size)
{
    idleThreadLock.Lock();
    long t = idleThreadList.size();
    if (t >= size)
    {
        for (long i = 0; i < size; i++)
        {
...
ThreadPool::Thread::Thread(ThreadPool *threadPool) : busy(FALSE),
