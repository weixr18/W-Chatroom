...
typedef std::function<int(void *)> TaskFun;         // 任务函数
typedef std::function<void *(int)> TaskCallbackFun; // 回调函数
...
public:
    ThreadPool(long minNumOfThread = 2, long maxNumOfThread = 10);
    ~ThreadPool();

    BOOL QueueTaskItem(TaskFun task, PVOID param, TaskCallbackFun taskCb = NULL, BOOL longFun = FALSE); // 任务入队

private:
    long getCurNumOfThread() { return getIdleThreadNum() + getBusyThreadNum(); } // 获取线程池中的当前线程数
    long GetMaxNumOfThread() { return maxNumOfThread - numOfLongFun; }           // 获取线程池中的最大线程数
    void SetMaxNumOfThread(long size)                                            // 设置线程池中的最大线程数
    {
    ...
    long GetMinNumOfThread() { return minNumOfThread; }          // 获取线程池中的最小线程数
    void SetMinNumOfThread(long size) { minNumOfThread = size; } // 设置线程池中的最小线程数

    long getIdleThreadNum() { return idleThreadList.size(); } // 获取线程池中的线程数
    long getBusyThreadNum() { return busyThreadList.size(); } // 获取线程池中的线程数
    void CreateIdleThread(long size);                         // 创建空闲线程
    void DeleteIdleThread(long size);                         // 删除空闲线程
    ...
    long maxNumOfThread;   // 线程池中最大的线程数
    long minNumOfThread;   // 线程池中最小的线程数
    long numOfLongFun;     // 线程池中最小的线程数
};
